...
def learn_inplace(binary_file_paths,
    ...
    cdef unsigned int n_vector_dimensions = outcome_vectors.shape[1]
    cdef char* fname
...
#                  weights_ptr, mm, n_vector_dimensions, all_outcomes_ptr, 0,
#                            length_all_outcomes)
#    if (error != 0):
#        raise IOError('binary files does not have proper format, error code %i' % error)
...
cdef int learn_inplace_ptr(char* binary_file_path, 
                        dtype_t* outcome_vectors,
                        dtype_t eta,
                        dtype_t* weights,
                        unsigned int mm,
                        unsigned int n_vector_dimensions,
    ...
    for event in range(number_of_events):
        ...
        for ii in range(start, end):
            ...
            index = n_vector_dimensions
            index *= all_outcome_indices[outcome_indices[0]]
            index += ii
            update = eta * (outcome_vectors[index] - association_strength)
            for jj in range(number_of_cues):
              index = mm  # implicit cast to unsigned long long
              index *=  ii  # this can't overflow anymore
              index += cue_indices[jj]  # this can't overflow anymore
